    ...
    for k in range(1, 999):
        for i in range(0, len(A[-1]['path']) - 1):
            ...
            if path_spur['path']:
                ...
                # check the SB path
                potential_path = potential_k['path']
                # set the flag
                a = 0
                for odd in range(0, len(potential_path) - 2, 2):
                    if potential_path[odd] == potential_path[odd+2]:
                        a += 1
                for even in range(1, len(potential_path) - 2, 2):
                    if potential_path[even] == potential_path[even+2]:
                        a += 1

                if not (potential_k in B) and a == 0:
